...
const getRandomWord = (length) => {
  ...
  const allWords = hotBoard[lengthMap[length] || lengthMap.sixLetterWords];
  return allWords[Math.floor(Math.random() * allWords.length)];
};

/**
 * Quick word -> definition/part-of-speech lookup
 */
const definitionMap = masterDictionary.reduce((accumulator, entry) => {
  accumulator[entry.word] = {
 ...
 * @param {Array<string>} wordList The words to look up
 * @returns {{ [word: string]: { definition: string, partOfSpeech: string } }}
 */
const definitionsFor = (wordList) => wordList.reduce((accumulator, word) => {
  if (definitionMap[word]) accumulator[word] = definitionMap[word];
  return accumulator;
}, {});
